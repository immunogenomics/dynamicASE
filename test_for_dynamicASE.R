...
bas <- read.table("/medpop/srlab/cd4tc_LS/rnaseq/both/waspGatkASE/summary_both_alleles_seen_swappedT4T8.txt", header = T, stringsAsFactors = F, row.names = 1)
 # reference allele counts per site
ref <- read.table("/medpop/srlab/cd4tc_LS/rnaseq/both/waspGatkASE/summary_refCount_swappedT4T8.txt", header = T, stringsAsFactors = F, row.names = 1)
  # alternative allele counts per site
alt <- read.table("/medpop/srlab/cd4tc_LS/rnaseq/both/waspGatkASE/summary_altCount_swappedT4T8.txt", header = T, stringsAsFactors = F, row.names = 1)
